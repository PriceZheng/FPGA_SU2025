...
from os import path
import glob
import shutil  # make sure this is at the top if not already
...
exit
    ...
    f = open(dic+script_name, 'w')
    f.write(script_content)
    f.close()

    shutil.rmtree(dic + 'project_tmp', ignore_errors=True)
    #subprocess.run(['vitis_hls', '-f', script_name], stderr=subprocess.STDOUT, cwd=dic)
    result = subprocess.run(['vitis_hls', '-f', script_name], shell=True, cwd=dic)
    ...
    if result.returncode != 0:
        print("[!] HLS process failed.")
        return -1   
    #if p.poll() == None:
    #    p.kill()
    #result.returncode != 0:
    ### collect the xml files for all the functions
    xml_file_path = dic+'project_tmp/solution_tmp/.autopilot/db/'
    ...
    for adb_file in glob.glob(xml_file_path + '*.adb'):
        ...
        print(func_name)
        shutil.copy(adb_file, path.join(dic, case_name + '_' + func_name + '.adb'))

    ### collect the implementation results
    syn_rpt_name = dic+'project_tmp/solution_tmp/syn/report/%s_csynth.xml' % top_func_name
    imp_rpt_name = dic+'project_tmp/solution_tmp/impl/report/verilog/%s_export.rpt' % top_func_name
    ...
    for ele in root.iter():
        if ele.tag == 'Average-caseLatency':
            if ele.text.isdigit():
                LAT = str(int(ele.text))
            else:
                LAT = 'undef'

    f_rpt = open(imp_rpt_name, 'r')
    SLICE = LUT = FF = DSP = CP = BRAM = 0
    DEVICE = ''
    for line in f_rpt.readlines():
            res = [i for i in line.split() if i.isdigit()]
            if line.startswith('Device target:'):
                    DEVICE = str([i for i in line.split()][2])
    ...
    HLS_sol = {}
    HLS_sol[DEVICE] = {}
    HLS_sol[DEVICE]['device'] = DEVICE
    HLS_sol[DEVICE]['name'] = case_name
    HLS_sol[DEVICE]['DSP'] = DSP
    HLS_sol[DEVICE]['LAT'] = LAT
    HLS_sol[DEVICE]['LUT'] = LUT
    HLS_sol[DEVICE]['CP'] = CP
    HLS_sol[DEVICE]['FF'] = FF
    HLS_sol[DEVICE]['SLICE'] = SLICE
    HLS_sol[DEVICE]['BRAM'] = BRAM
